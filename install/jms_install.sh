...
if [ "$(docker ps -a | grep jms_guacamole | grep $Version)" == "" ] || [ "$flag" == "1" ]; then
    ...
    docker run --name jms_guacamole -d -p 127.0.0.1:8081:8080 -e JUMPSERVER_SERVER=http://$Server_IP:8080 -e BOOTSTRAP_TOKEN=$BOOTSTRAP_TOKEN -e GUACAMOLE_HOME=/config/guacamole -e GUACAMOLE_LOG_LEVEL=ERROR --restart=always wojiushixiaobai/jms_guacamole:$Version
else
