...
if [ ! -d "/config/tomcat9" ]; then
    if [ ! -f "/config/apache-tomcat-9.0.29.tar.gz" ]; then
        wget -O /config/apache-tomcat-9.0.29.tar.gz https://mirrors.tuna.tsinghua.edu.cn/apache/tomcat/tomcat-9/v9.0.29/bin/apache-tomcat-9.0.29.tar.gz || {
            rm -rf /config/apache-tomcat-9.0.29.tar.gz
            wget -O /config/apache-tomcat-9.0.29.tar.gz https://demo.jumpserver.org/download/apache/tomcat/tomcat-9/v9.0.29/bin/apache-tomcat-9.0.29.tar.gz
        }
    fi
    tar xf /config/apache-tomcat-9.0.29.tar.gz -C /config
    mv /config/apache-tomcat-9.0.29 /config/tomcat9
